...
class ArtifactCommandsTest : FunSpec({
    ...
    test("artifact import posts binary data with correct Content-Type") {
        ...
        val tmpFile = Files.createTempFile("project", ".zip").toFile()
        try {
    ...
    test("artifact get makes GET to /api/v1/projects/{id}/artifacts/{path} and prints content") {
        ...
        try {
            ...
            path shouldBe "/api/v1/projects/run-1/artifacts/live.log"
            output shouldContain "log content"
        } finally { srv.stop(0) }
    }

    test("artifact stage-log makes GET to /api/v1/projects/{id}/stages/{nodeId}/log and prints content") {
        var path: String? = null
        val (srv, port) = startFakeServer { ex ->
            path = ex.requestURI.path
        ...
        try {
            ...
            path shouldBe "/api/v1/projects/run-1/stages/writeTests/log"
            output shouldContain "stage output"
        } finally { srv.stop(0) }
    }

    test("artifact failure-report makes GET to /api/v1/projects/{id}/failure-report and prints JSON") {
        var path: String? = null
        val (srv, port) = startFakeServer { ex ->
            path = ex.requestURI.path
        ...
        try {
            ...
            path shouldBe "/api/v1/projects/run-1/failure-report"
    ...
    test("artifact export writes ZIP bytes to file and prints confirmation") {
        ...
        try {
            val outFile = java.io.File(tmpDir, "project-run-1.zip")
